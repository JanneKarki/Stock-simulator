...
class StockRepository:
    ...
    def add_to_portfolio(self, user, stock, price, amount):
        ...
        if len(data) == 0:  # jos 0 niin ei ole tietokannassa
            stocks_database.execute("INSERT INTO Stocks (user,content,avg_price,amount) values (?,?,?,?)", [
                                    user, stock, price, amount])
        else:
            print("on")
            new_amount = data[0][1]+amount
            new_avg_price = ((data[0][0]*data[0][1])+(amount*price))/new_amount
            stocks_database.execute("UPDATE Stocks SET avg_price = ? where user = ? and content = ?", [
                                    new_avg_price, user, stock])
            stocks_database.execute("UPDATE Stocks SET amount = ? where user = ? and content = ?", [
